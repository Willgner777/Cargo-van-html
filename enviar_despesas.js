...
async function buscarItens(token) {
  ...
  let url =
    `https://graph.microsoft.com/v1.0/sites/${siteId}/lists/${LISTA_DESPESAS}/items?expand=fields&$top=1000`;
...
(async () => {
  try {
    ...
    for (const it of itens) {
      ...
      if (
        st.includes("recusado") ||
        st.includes("rejeitado") ||
        st.includes("cancelado")
      ) {
        recusados++;
      } else if (st.includes("aprovado")) {
        aprovados++;
      } else {
        pendentes++;
      }
